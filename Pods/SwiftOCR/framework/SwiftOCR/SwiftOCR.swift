...
open class SwiftOCR {
    ...
    internal func extractBlobs(_ image: OCRImage) -> [(OCRImage, CGRect)] {
        ...
        for label in minMaxXYLabelDict.keys {
            ...
            let notToTall    = true //Double(maxY - minY) < Double(imageHeight!) * 0.75
            let notToWide    = true //Double(maxX - minX) < Double(imageWidth ) * 0.25
            let notToShort   = true //Double(maxY - minY) > Double(imageHeight!) * 0.08
            let notToThin    = true //Double(maxX - minX) > Double(imageWidth ) * 0.01
